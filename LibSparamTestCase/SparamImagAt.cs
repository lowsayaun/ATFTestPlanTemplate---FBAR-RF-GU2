...
{
    public class SparamImagAt : SparamTestCase.TestCaseAbstract 
    {
        ...
        public override void RunTest()
        {
            ...
            if (usePrevious > 0)
            {
                ...
                rtnResult = tmpSparamRaw[ChannelNumber - 1].SParamData[GetSparamIndex()].SParam[i_TargetFreqCnt].Imag;
            }
            else
            {
                ...
                if (blnInterStart)
                {
                    double p2 = tmpSparamRaw[ChannelNumber - 1].SParamData[GetSparamIndex()].SParam[iTargetCnt + 1].Imag;
                    double p1 = tmpSparamRaw[ChannelNumber - 1].SParamData[GetSparamIndex()].SParam[iTargetCnt].Imag;
                    rtnResult = p1 + (partialGradient * (p2 - p1));
                }
                else
                {
                    rtnResult = tmpSparamRaw[ChannelNumber - 1].SParamData[GetSparamIndex()].SParam[iTargetCnt].Imag;
                }
            }
